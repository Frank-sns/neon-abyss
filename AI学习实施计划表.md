...
> 起点：2026年7月21日（周二）
> 背景：自动化本科，数学/控制基础扎实，编程零基础，研0暑期
> 目标：2027年1月前能独立跑通 DQN + CartPole，看懂课题组代码
...
> 资源：B站「黑马程序员 Python+AI 零基础入门」[BV1sHU9BmEne](https://www.bilibili.com/video/BV1sHU9BmEne)
> 方法：边看边敲，老师写一行你敲一行
...
> 📚 NumPy 参考：菜鸟教程 [runoob.com/numpy](https://www.runoob.com/numpy/numpy-tutorial.html)
...
> 📚 sklearn 参考：官方中文文档 [sklearn.apachecn.org](https://sklearn.apachecn.org/)

#### 第 9 周：scikit-learn（下）— 扩展 + 综合
...
> 📚 PyTorch 参考：李沐《动手学深度学习》[zh.d2l.ai](https://zh.d2l.ai/) 第2章 | B站视频

#### 第 11 周：PyTorch（中）— 线性模型 + 多层感知机
...
**背景**：一个带死区的二阶系统，输入 u(t)，输出 y(t)，有测量噪声。
传统线性辨识做不到完全拟合，用神经网络来做。

**步骤**：
1. 用 NumPy 生成仿真数据（输入随机信号 → 二阶系统 + 非线性死区 → 加噪声 → 输出）
2. 用 pandas 整理数据，划分训练/测试集
3. 用 PyTorch 搭建一个 3~4 层 MLP
4. 写训练循环，画 loss 下降曲线
5. 在测试集上对比预测 vs 真值，计算 R²
6. 用 Matplotlib 画完整对比图（时域曲线 + 散点对比图）
7. 整理成 Jupyter notebook，push 到 GitHub

**做完这个项目，你就正式跨入了"会用 AI 工具做控制相关实验"的门槛。**

---

### 🚀 第三阶段：方向深耕（2026.10.14 起）
...
| 第 15 周 | **10.28~11.3** | CartPole 实战：PyTorch 官方 DQN 教程，逐行理解 | [pytorch.org/tutorials](https://pytorch.org/tutorials/intermediate/reinforcement_q_learning.html) |
...
> 💡 保持**每天都有产出**（哪怕只有 10 行代码），比周末突击 6 小时效果好得多。
...
### 第一阶段完成标准（8.18 前）
- [ ] 能独立写出 50 行以上的 Python 脚本（不查资料）
- [ ] 至少完成 3 个与控制相关的练习（一阶/二阶系统计算）
- [ ] 代码已用 Git 管理，push 到了 GitHub

### 第二阶段完成标准（10.13 前）
- [ ] NumPy/Matplotlib/pandas 各完成了至少 2 个练习
- [ ] 用 sklearn 完整跑过一个 ML pipeline（数据→训练→评估）
- [ ] 综合项目（神经网络拟合非线性系统）完成并 push 到 GitHub
- [ ] 会使用 Jupyter notebook 做实验
- [ ] 会用 conda 创建/管理虚拟环境

### 第三阶段中期标准（11.24 前）
- [ ] DQN CartPole 跑通，杆子稳定直立超过 200 步
- [ ] 能用自己的话解释 DQN 的原理（经验回放、目标网络、ε-greedy）
- [ ] 至少阅读了 1 篇课题组相关论文
...
> 🚫 不要让报错中断你超过 30 分钟，超过就先跳过，第二天再回来解决。
...
> 📌 **核心提醒**：这个计划的目的是让你**有节奏地前进**，不是给你制造焦虑。
> 如果某周没完成，下周补上就行；如果提前完成，就提前进入下一阶段。
> **重要的是每天都在前进，哪怕只前进一点点。**
